...
| UserDefined     | String | Up to 50 key value pairs can be stored in this object. Below pair has ability to disable PayFabric's email receipt to go out automatically for the corresponding transaction, {"Name": "DisableEmailReceipt","Value": "true"} If True, this will disable the email receipt to go out for the corresponding transaction.If False, this will enable the email receipt to go out for the corresponding transaction. But this still depends on the prerequisite that email receipt is enabled on PayFabric service portal.|
