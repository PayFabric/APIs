...
| UserDefined     | String | Up to 50 key value pairs can be stored in this object. Note: Adding `DisableEmailReceipt` under `UserDefined` to disable PayFabric's email receipt to be sent out for the processed transaction, the possible values are `True` and `False`. If the value is `True`, this will disable the email receipt to be sent out out for the processed transaction. If if the value is `False`, this will enable the email receipt to be sent out for the processed transaction based on the configuration of [Payment Receipt](https://github.com/PayFabric/Portal/blob/master/PayFabric/Sections/Payment%20Receipt.md).|
