...
| Status          | String | Status message indicating the status of the transaction. Possible values are ``Approved``, ``Denied``, ``Declined``, ``Failure``, ``AVSFailure``,  ``MoreInfo``,``ThreeDSChallenge``,``ServerError`` and ``FraudReject``. |varchar(16)|  
