...
public void CreateToken(string username, string password, string URL, ref Token token)
{
	...
	// ------------------------------------------------------
	// Go to https://github.com/PayFabric/APIs/blob/master/Receivables/Sections/APIs/API/Token.md#payfabric-receivables-api-authentication for more details about request and response.
	// ------------------------------------------------------	
	...
	try
	{
		...
		request.AddParameter("application/x-www-form-urlencoded", "grant_type=password&client_id=ePay_Customer_Portal&username=" + username + "&password=" + password, ParameterType.RequestBody);
