...
* `GET /rest/api/wallet/get/John+Doe+Ltd?tender=CreditCard` or GET `/rest/api/wallet/getByCustomer?customer=John+Doe+Ltd&tender=CreditCard` will return the credit cards or eChecks for a customer depending on the tender type (_CreditCard_, _ECheck_), the latter support special characters.
